(function () {
  ...
  function isSensitivePropKey(key) {
    ...
    var aggregateAllowList = /^(action|tool|target|tab|status|scope|export|format|checklist|category|checked|people|days|score_level|score_bucket|label|has_value|inventory_count|family_fields|mode|network|region|source_status|source)$/;
  ...
  function actionKind(text) {
    ...
    if (/compress|resize|convert|generate|remove|merge|split|extract|calculate|estimate|test|start|run|create|scan|analyze|clean|format|render|reset|pack|inventory|readiness|pulse|refresh|watch|pantry|recall|shortage|watch|pantry|recall|shortage/.test(text)) return "tool_action";
    return "";
  }
